...
Процедура СохранитьНастройкиОбработки(ПолноеИмя, ИзменятьВТранзакции, ПрерыватьПриОшибке, ТОНастройкаПорции,
...
Процедура ЗагрузитьНастройкиОбработки()
	...
	СтруктураНастроек = ХранилищеОбщихНастроекЗагрузить(
		"Обработка.ГрупповоеИзменениеОбъектов", ВидыИзменяемыхОбъектов);
...
Процедура РазрешитьРедактированиеРеквизитов(ВыбранныйРеквизит)
	...
	Иначе
		...
		Если ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			ЗапретРедактированияРеквизитовОбъектовКлиентМодуль = ОбщийМодуль(
...
Функция ПолучитьФильтрРедактированияПоТипу(ОбъектМетаданных)
	...
	ФильтрПоТипуОМ = ФильтрТаблица.НайтиСтроки(
							Новый Структура("ObjectType", ИмяБазовогоТипаПоОбъектуМетаданных(ОбъектМетаданных)));
		...
		Если Не ПустаяСтрока(Пояснение) Тогда
			...
			Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
				Если ЕстьОтборПоСтрокам Тогда
					Пояснение = Пояснение + " " + НСтр(
						"ru = 'Изменения будут выполнены в тех строках выбранных элементов, которые удовлетворяют <a href = ""ПерейтиКНастройкеОтбора"">условиям отбора</a>.'");
				Иначе
					Пояснение = Пояснение + " " + НСтр(
						"ru = 'Изменения будут выполнены <a href = ""ПерейтиКНастройкеОтбора"">во всех строках</a> выбранных элементов.'");
				КонецЕсли
				;
			КонецЕсли;
		Иначе
...
Процедура СохранитьНастройки()
	СохранитьНастройкиОбработки(
			"", Объект.ИзменятьВТранзакции, Объект.ПрерыватьПриОшибке, ТОНастройкаПорции, ТОПроцентОбъектовВПорции,
		ТОЧислоОбъектовВПорции, ОбрабатыватьРекурсивно);
КонецПроцедуры
