...
Процедура ДобавитьТренд(Команда)
	Если Не ЗначениеЗаполнено(ТипАппроксимации) Тогда
		ПоказатьПредупреждение(Новый ОписаниеОповещения("НеЗаполненыДанные", ЭтотОбъект), "Не выбран тип аппроксимации",
			, "Ошибка");
		Возврат;
	КонецЕсли;
	Добавили = Ложь;
	...
	Если Не Добавили Тогда
		ПоказатьПредупреждение(Новый ОписаниеОповещения("НеЗаполненыДанные", ЭтотОбъект), "Не выбран ни одна серия", ,
			"Ошибка");
		Возврат;
	КонецЕсли;
КонецПроцедуры
...
Процедура СформироватьНаСервере()
	...
	Иначе
		...
		|	ДанныеПродаж.Период МЕЖДУ НАЧАЛОПЕРИОДА(&Начало, МЕСЯЦ) И КОНЕЦПЕРИОДА(&Конец, МЕСЯЦ)
		|	И ДанныеПродаж.Номенклатура В(&МассивНоменклатуры)
		|	И ДанныеПродаж.Клиент.ВИП = ЛОЖЬ
		|
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ДанныеПродаж.Период, МЕСЯЦ),
		|	ДанныеПродаж.Номенклатура
		...
		Пока ВыборкаПериод.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				тчк = СокрП(Сред(
					...
					Месяц * 8 + 1, 3)) + формат(Выборка.Период, "ДФ=' yy'");
